...
/**
 * Email: Manual mode — issue found, here are the details + AI comment to paste
 */
export async function emailManualClaim(issue, suggestedComment) {
  const commentSection = suggestedComment
    ? `
        <div style="margin-bottom: 16px;">
          <p style="margin: 0 0 8px; color: #8b949e; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">💬 AI-Suggested Comment (copy &amp; paste):</p>
          <div style="background: #010409; border: 1px solid #30363d; border-left: 3px solid #58a6ff; padding: 14px 16px; border-radius: 0 6px 6px 0;">
            <p style="margin: 0 0 10px; color: #c9d1d9; font-size: 14px; line-height: 1.7; white-space: pre-wrap;">${suggestedComment.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>
            <p style="margin: 0; color: #484f58; font-size: 11px;">↑ Generated by Gemini AI — feel free to edit before posting</p>
          </div>
        </div>`
    : `<div style="background: #1c1a0d; border: 1px solid #9e6a03; border-radius: 8px; padding: 12px 16px; margin-bottom: 16px;">
         <p style="margin: 0; color: #d29922; font-size: 13px;">⚠️ Could not generate AI comment — write your own below.</p>
       </div>`;

  await sendEmail({
    subject: `🎯 New Issue #${issue.number}: ${issue.title} — Action Required`,
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 640px; margin: 0 auto; padding: 20px; background: #0d1117; color: #c9d1d9; border-radius: 12px;">

        <div style="background: linear-gradient(135deg, #1f3a6e 0%, #1f6feb 100%); padding: 20px; border-radius: 8px; margin-bottom: 20px;">
          <h1 style="margin: 0; color: #ffffff; font-size: 22px;">🎯 New Good-First-Issue!</h1>
          <p style="margin: 8px 0 0; color: #cae8ff; font-size: 14px;">Manual mode — comment yourself to claim it fast ⚡</p>
        </div>

        <!-- Issue details -->
        <div style="background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 20px; margin-bottom: 16px;">
          <h2 style="margin: 0 0 10px; color: #58a6ff; font-size: 18px; line-height: 1.3;">
            <a href="${issue.html_url}" style="color: #58a6ff; text-decoration: none;">#${issue.number} — ${issue.title}</a>
          </h2>

          <!-- Labels -->
          <div style="margin-bottom: 14px; display: flex; flex-wrap: wrap; gap: 6px;">
            ${issue.labels.map((l) => `<span style="background: #1f6feb22; color: #58a6ff; border: 1px solid #1f6feb55; padding: 2px 10px; border-radius: 20px; font-size: 12px; display: inline-block;">${l.name}</span>`).join(" ")}
          </div>

          <!-- Meta row -->
          <table style="width: 100%; border-collapse: collapse; margin-bottom: 14px;">
            <tr>
              <td style="padding: 4px 0; color: #8b949e; font-size: 12px; width: 100px;">Created</td>
              <td style="padding: 4px 0; color: #c9d1d9; font-size: 12px;">${new Date(issue.created_at).toUTCString()}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; color: #8b949e; font-size: 12px;">Comments</td>
              <td style="padding: 4px 0; color: #c9d1d9; font-size: 12px;">${issue.comments}</td>
            </tr>
            <tr>
              <td style="padding: 4px 0; color: #8b949e; font-size: 12px;">State</td>
              <td style="padding: 4px 0; color: #3fb950; font-size: 12px; font-weight: 600;">● ${issue.state}</td>
            </tr>
          </table>

          <!-- Description -->
          <p style="margin: 0 0 6px; color: #8b949e; font-size: 12px; text-transform: uppercase; letter-spacing: 0.05em;">Description:</p>
          <p style="margin: 0; color: #c9d1d9; font-size: 14px; line-height: 1.7; background: #010409; padding: 12px; border-radius: 6px; border: 1px solid #21262d;">
            ${issue.body ? issue.body.substring(0, 800).replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\n/g, "<br>") + (issue.body.length > 800 ? "<br><em style='color:#484f58'>... (truncated)</em>" : "") : "<em style='color:#484f58'>No description provided.</em>"}
          </p>
        </div>
        ...
        <!-- CTA buttons -->
        <div style="text-align: center; margin: 20px 0 8px; display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
          <a href="${issue.html_url}#partial-timeline" style="background: #1f6feb; color: white; padding: 13px 28px; border-radius: 7px; text-decoration: none; font-weight: 600; font-size: 14px; display: inline-block;">
            💬 Comment on GitHub →
          </a>
          <a href="${issue.html_url}" style="background: #21262d; color: #c9d1d9; padding: 13px 28px; border-radius: 7px; text-decoration: none; font-weight: 600; font-size: 14px; display: inline-block; border: 1px solid #30363d;">
            🔗 View Full Issue
          </a>
        </div>

        <p style="margin-top: 20px; color: #484f58; font-size: 12px; text-align: center;">
          Spotted at ${new Date().toISOString()} • OpenMetadata Issue Catcher • AUTO_COMMENT=no
        </p>
      </div>
    `,
  });
